...
import { Award, BookOpen, ChevronUp, ClipboardList, Crown, FileCheck2, GraduationCap, Home, Languages, LayoutDashboard, Loader2, LogOut, Menu, MessageSquare, Package, Settings, ShoppingBag, X } from "lucide-vue-next"
...
<template>
  ...
  <div v-if="mobileMenuOpen" class="app-viewport-height fixed inset-0 z-50 lg:hidden">
    ...
    <aside class="app-side-card app-safe-area-sidebar absolute left-0 top-0 flex h-full w-[280px] max-w-[82vw] max-h-none flex-col overflow-y-auto rounded-none border-r border-sidebar-border bg-sidebar shadow-2xl shadow-slate-950/20">
      <div class="flex h-20 items-center justify-between border-b border-white/10 px-5">
        ...
        <button class="flex h-11 w-11 cursor-pointer items-center justify-center rounded-full text-white/70 transition-colors hover:bg-white/10 hover:text-white" @click="mobileMenuOpen = false">
          <X class="h-5 w-5" />
        </button>
      </div>
      ...
      <div class="px-4 pb-5">
        <button
          class="mb-3 flex h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-full border border-white/25 bg-transparent px-4 text-sm font-semibold text-white transition-colors hover:border-white/50 hover:bg-white/10"
          type="button"
          @click="changeLanguage(lang === 'zh' ? 'en' : 'zh')"
        >
          <Languages class="h-4 w-4" />
        ...
        <button
          class="flex h-12 w-full items-center gap-4 rounded-lg px-4 text-left text-white transition-colors hover:bg-white/10"
          :class="menuOpen ? 'bg-white/10' : ''"
          type="button"
          @pointerdown.stop
          @click="menuOpen = !menuOpen"
        >
          <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-sm font-bold text-[#002a66]">
            {{ userName.charAt(0).toUpperCase() }}
          </span>
          <span class="min-w-0 flex-1 truncate text-sm font-medium text-white">{{ userName }}</span>
          <ChevronUp class="h-4 w-4 shrink-0 text-white/70 transition-transform" :class="menuOpen ? '' : 'rotate-180'" :stroke-width="1.5" />
        </button>
      </div>
    </aside>
  </div>
  ...
  >
    ...
    <div :class="[isSidebarCollapsed ? 'px-0' : 'px-5', 'pb-6']">
      <button
        :class="[
          'mb-3 flex h-9 w-full cursor-pointer items-center justify-center gap-2 border border-white/25 bg-transparent text-sm font-semibold text-white transition-colors hover:border-white/50 hover:bg-white/10',
          isSidebarCollapsed ? 'mx-auto w-9 rounded-full px-0' : 'rounded-full px-4',
        ]"
        type="button"
        @click="changeLanguage(lang === 'zh' ? 'en' : 'zh')"
      ...
      <button
        :class="[
          'flex h-12 w-full items-center rounded-lg text-left text-white transition-colors hover:bg-white/10',
          isSidebarCollapsed ? 'mx-auto h-9 w-9 justify-center px-0' : 'gap-4 px-4',
      ...
      >
        ...
        <span v-if="!isSidebarCollapsed" class="min-w-0 flex-1 truncate text-sm font-medium text-white">{{ userName }}</span>
        <ChevronUp v-if="!isSidebarCollapsed" class="h-4 w-4 shrink-0 text-white/70 transition-transform" :class="menuOpen ? '' : 'rotate-180'" :stroke-width="1.5" />
      </button>
    </div>
  </aside>

  <div ref="menuContainer" class="fixed bottom-5 left-5 z-50">
    <div v-if="menuOpen" class="mb-2 w-[240px] overflow-hidden rounded-lg border border-border bg-white shadow-lg shadow-slate-950/10 lg:mb-[60px]">
      <RouterLink
      ...
      <RouterLink
        ...
        @click="menuOpen = false"
      >
        <span class="flex h-8 w-full items-center gap-3 rounded-lg px-3 transition-colors group-hover/menu-row:bg-primary group-hover/menu-row:text-white">
          <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-slate-100 text-slate-500 transition-colors group-hover/menu-row:bg-white/20 group-hover/menu-row:text-white">
            <Settings class="h-5 w-5" :stroke-width="2.25" />
          </span>
      ...
      <button class="group/menu-row flex h-12 w-full cursor-pointer items-center border-t border-border px-2 text-left text-sm text-slate-900 disabled:cursor-not-allowed disabled:opacity-70" :disabled="logoutLoading" @click="handleLogout">
        <span class="flex h-8 w-full items-center gap-3 rounded-lg px-3 transition-colors group-hover/menu-row:bg-primary group-hover/menu-row:text-white">
          <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-slate-100 text-slate-500 transition-colors group-hover/menu-row:bg-white/20 group-hover/menu-row:text-white">
            <Loader2 v-if="logoutLoading" class="h-5 w-5 animate-spin" :stroke-width="2.25" />
            <LogOut v-else class="h-5 w-5" :stroke-width="2.25" />
          </span>
