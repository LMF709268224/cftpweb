      ...
      <div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
        ...
        <div v-if="paymentPreview" class="rounded-lg border border-border bg-muted/30 p-4 transition-opacity duration-200">
          ...
          <div class="space-y-2 text-sm">
            ...
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ copy.tax }}</span>
              <span class="font-medium">{{ formatMoney(paymentPreview.tax_total, paymentPreview.currency) }}</span>
            </div>
          ...
          <div v-if="paymentPreview.breakdown?.length" class="mt-3 overflow-hidden rounded-xl border border-emerald-200/80 bg-white shadow-sm shadow-emerald-100/60">
            ...
            <div class="divide-y divide-emerald-100">
              <div v-for="item in paymentPreview.breakdown" :key="`${item.code}-${item.discount}`" class="flex items-start justify-between gap-3 px-3 py-2.5 text-xs">
                ...
                <div class="shrink-0 rounded-full bg-emerald-100 px-2.5 py-1 font-bold text-emerald-800">-{{ formatMoney(item.discount, paymentPreview.currency) }}</div>
              </div>
            </div>
          </div>
