...
watch(
  () => [props.activeCouponCodes.length, props.error, props.cannotPayReason] as const,
  ([activeCouponCount, error, cannotPayReason]) => {
    if (activeCouponCount || error || cannotPayReason) expanded.value = true
  },
  { immediate: true },
)
...
<template>
  <div>
    <button
      type="button"
      class="inline-flex items-center gap-1 text-sm font-medium text-primary underline-offset-4 transition-colors hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
      :aria-expanded="expanded"
      aria-controls="purchase-coupon-panel"
      @click="expanded = !expanded"
    >
      {{ copy.couponToggle }}
      <ChevronDown :class="['h-4 w-4 transition-transform', expanded && 'rotate-180']" aria-hidden="true" />
    </button>

    <div v-if="expanded" id="purchase-coupon-panel" class="mt-2 rounded-lg border border-blue-100 bg-blue-50/70 p-4">
