...
function quizCompleted(quizId?: string, quizType: number = 1, unitStatus?: any) {
  ...
  if (quizType === 2) {
    ...
    return p.is_passed || s === "3" || s.includes("COMPLETED")
  }
  return Boolean(p.is_passed)
}
              ...
              <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="min-w-0 space-y-3">
                  ...
                  <div class="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                    ...
                    <div v-if="shouldShowStoredExamDetails(exam) && hasText(exam.appointment_start_time)"><span class="font-medium text-foreground">{{ t.examsPage.appointmentStart }}:</span> {{ formatBackendDate(exam.appointment_start_time, lang) }}</div>
                    <div v-if="shouldShowStoredExamDetails(exam) && hasText(exam.appointment_end_time)"><span class="font-medium text-foreground">{{ t.examsPage.appointmentEnd }}:</span> {{ formatBackendDate(exam.appointment_end_time, lang) }}</div>
