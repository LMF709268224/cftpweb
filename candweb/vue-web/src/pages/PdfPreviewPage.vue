...
<template>
  <div v-if="isResourcePackPreview" class="candidate-portal app-viewport-height app-safe-area-screen flex flex-col overflow-hidden bg-background">
    <header class="shrink-0 px-3 py-3 lg:px-8 lg:pb-16 lg:pt-8">
    ...
    <main class="grid min-h-0 flex-1 grid-rows-[auto_minmax(0,1fr)] gap-3 px-3 pb-3 lg:grid-cols-[minmax(0,1fr)_468px] lg:grid-rows-1 lg:gap-8 lg:px-8 lg:pb-4">
      <section class="order-2 min-h-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:order-none">
      ...
      <aside class="order-1 h-fit min-w-0 self-start overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:order-none lg:p-8">
        <h1 class="truncate text-base font-bold leading-tight text-slate-950 lg:overflow-visible lg:whitespace-normal lg:text-clip lg:text-xl" :title="resourceDetailTitle">
          {{ resourceDetailTitle }}
        </h1>
        <div v-if="resourceDetailDate" class="mt-2 flex items-center gap-2 text-sm text-slate-500 lg:mt-6 lg:gap-3">
          <Calendar class="h-4 w-4 text-slate-600" />
          <span class="min-w-0 truncate">{{ resourceDetailDate }}</span>
        </div>
      </aside>
    </main>
  </div>
  ...
  <div v-else class="candidate-portal app-viewport-height app-safe-area-screen flex flex-col bg-background">
    <header class="flex h-16 shrink-0 items-center border-b border-slate-200 bg-white px-3 shadow-sm sm:px-4">
