...
<template>
  <div class="ecosystem-page">
    ...
    <main v-else-if="isMembership" class="membership-page">
      <section class="membership-hero"><GfiLineBackground /><div class="section-container" data-reveal><h1>{{ lang === "zh" ? "GFI会员资格计划" : "GFI Membership Programme" }}</h1><p>{{ lang === "zh" ? "在全球金融科技学院，会员资格不仅仅是订阅——这是对专业卓越的承诺，是金融科技领域的终身职业旅程。我们的分层会员资格结构支持从探索行业的学生到塑造未来的高级专业人士的每个阶段。" : "At the Global Fintech Institute, membership is more than a subscription—it is a commitment to professional excellence and a lifelong career journey in fintech. Our tiered membership structure supports every stage, from students exploring the industry to senior professionals shaping its future." }}</p></div></section>
