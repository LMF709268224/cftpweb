...
test("会员商品展示升级、在途订单和前置资格 blocker", async ({ page }) => {
    ...
    const missingQualificationBundle = {
        ...membershipBundle,
    ...
    await installCandidateApiMocks(page, ({ pathname, method }) => {
        if (pathname === "/api/mall/bundles" && method === "GET") {
            return { data: { bundles: [upgradeBundle, pendingBundle, missingQualificationBundle] } }
        }
        ...
        if (pathname === `/api/mall/bundles/${missingQualificationBundleID}` && method === "GET") return { data: missingQualificationBundle }
        if (pathname.endsWith("/pricing-detail")) return { data: {} }
        return undefined
    })
...
test("已持有有效资格的课程自动免考且不可取消", async ({ page }) => {
    ...
    const enrollmentFeeItem = page.locator(`[data-testid="checkout-included-item"][data-item-id="${pipelineID}"]`)
    await expect(enrollmentFeeItem).toContainText("报名费")
    ...
    const reviewLineItems = page.getByTestId("checkout-review-line-items")
    await expect(reviewLineItems.getByTestId("checkout-review-line-item")).toHaveCount(5)
    await expect(reviewLineItems.locator(`[data-item-id="${pipelineID}"]`)).toContainText("Enrollment fee")
    await expect(reviewLineItems.locator(`[data-item-id="${unitID}"]`)).toContainText("Exemption recognition fee")
    await expect(reviewLineItems.locator('[data-item-id="unit-fintech"]')).toContainText("L1B Fintech")
...
test("系统资格、自动免考和按原价购买以管线维度提交完整决定", async ({ page }) => {
    ...
    await page.getByTestId("checkout-next").click()
    await expect(page.getByTestId("checkout-step-review")).toBeVisible()
    await expect(page.getByTestId("checkout-full-payment-discount-badge")).toContainText("额外9折")
    const fullPaymentDiscountNotice = page.getByTestId("checkout-full-payment-discount-notice")
    await expect(fullPaymentDiscountNotice).toContainText("一次性全额支付优惠")
    await expect(fullPaymentDiscountNotice).toContainText("支付时自动抵扣")
    await expect(page.getByTestId("checkout-step-review").getByText("优惠前总计", { exact: true })).toBeVisible()
    const stagePaymentOption = page.getByTestId("checkout-stage-payment-option")
    const stagePaymentToggle = page.getByTestId("checkout-stage-payment-toggle")
    await expect(stagePaymentOption).toBeHidden()
    await expect(stagePaymentToggle).toHaveAttribute("aria-expanded", "false")
    await stagePaymentToggle.click()
    await expect(stagePaymentOption).toBeVisible()
    await expect(stagePaymentToggle).toHaveAttribute("aria-expanded", "true")
    await stagePaymentOption.locator('input[type="radio"][value="BY_STAGE"]').check()
    await expect(page.getByTestId("checkout-step-review").getByText(/\$650/, { exact: true }).last()).toBeVisible()
    await expect(fullPaymentDiscountNotice).toHaveCount(0)
    await expect(page.getByTestId("checkout-step-review").getByText("总计", { exact: true })).toBeVisible()
...
test("资格申请记录详情显示完整审核备注和已上传文件", async ({ page }) => {
    ...
    await expect(approvedRow).toContainText("2026年8月29日")
...
test("免考选择完成后按资格创建独立订单，已拒绝资格仍可重新申请或按原价购买", async ({ page }) => {
    ...
    await expect(page.getByText("这里仅展示申请要求和官方模板。请先点击申请该项免考并上传相关证明材料。", { exact: true })).toBeVisible()
    await selectionNextButton.click()
