...
export default function CredentialsPage() {
  ...
  const getStatusText = (status: string) => {
    return statusLabel(t, CANDIDATE_APPLICATION_STATUS_LABELS, status, "credentialsPage.appStatusUnknown")
  }
                ...
                <div className="overflow-hidden rounded-md border bg-card">
                  <div className="divide-y">
                    {applications.map(app => {
                      ...
                      const appStatusName = statusEnumNameForStatus(CANDIDATE_APPLICATION_STATUS_ENUM_NAMES, app.status).toUpperCase()
                      const canResubmit = ["REUPLOAD", "RESUBMIT", "NEEDS_RESUBMIT", "APPLICATION_STATUS_REUPLOAD", "APPLICATION_STATUS_RESUBMIT"].includes(appStatusName)
                      ...
                      return (
                        <div key={app.app_id} className="grid grid-cols-[minmax(180px,1.5fr)_minmax(220px,2fr)_minmax(120px,1fr)_auto] items-center gap-4 px-4 py-3 text-sm">
                          ...
                          <Badge variant="outline" className={`w-fit gap-1 ${statusBadgeClassForStatus(CANDIDATE_APPLICATION_STATUS_ENUM_NAMES, app.status)}`}>
