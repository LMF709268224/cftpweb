...
const normalizedExamStatus = (status?: string | number | null) =>
  normalizeEnumValueUpper(status);

const shouldShowExamStatus = (status?: string | number | null) => {
  const normalized = normalizedExamStatus(status);
  return Boolean(
    normalized &&
    !["NONE", "UNKNOWN", "UNSPECIFIED"].some((item) =>
...
> = {
  current: {
    titleKey: "noExams",
    descriptionKey: "noExamsDesc",
    icon: AlertCircle,
  },
  history: {
    titleKey: "noHistory",
    descriptionKey: "noHistoryDesc",
    icon: History,
  },
  exemption: {
    titleKey: "noExemption",
    descriptionKey: "noExemptionDesc",
    icon: ShieldCheck,
  },
  records: {
    titleKey: "noRecords",
    descriptionKey: "noRecordsDesc",
    icon: ClipboardList,
  },
};
...
export default function ExamsPage() {
  ...
  const [scheduleLoadingExamId, setScheduleLoadingExamId] = useState<
    string | null
  >(null);
  ...
  const handleScheduleExam = async (exam: ExamItem) => {
    ...
    try {
      ...
      const res = await apiClient(
        `/api/exams/${encodeURIComponent(exam.exam_id)}/schedule-url?${params.toString()}`,
      );
      if (res?.url) {
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {
                    t.examsPage[
                      tab.labelKey as keyof typeof t.examsPage
                    ] as string
                  }
                </button>
              );
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="mb-2 text-lg font-semibold text-foreground">
                  {
                    t.examsPage[
                      state.titleKey as keyof typeof t.examsPage
                    ] as string
                  }
                </h3>
                <p className="max-w-md text-muted-foreground">
                  {
                    t.examsPage[
                      state.descriptionKey as keyof typeof t.examsPage
                    ] as string
                  }
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>
                    {t.examsPage.countPrefix}{" "}
                    {total > 0 ? total : filtered.length}{" "}
                    {t.examsPage.countSuffix}
                  </span>
                  <span>
                    {activeTab === "history"
                      ? t.examsPage.historyFilterHint
                      : t.examsPage.visibleRecordsHint}
                  </span>
                </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            {shouldShowExamStatus(exam.exam_status) && (
                              <Badge
                                variant="outline"
                                className={statusBadgeClassForStatusValue(
                                  exam.exam_status,
                                )}
                              >
                                {examStatusLabel(t, exam.exam_status)}
                              </Badge>
                            )}
                            {shouldShowExamStatus(exam.result_status) && (
                              ...
                              >
                                {examStatusLabel(t, exam.result_status)}
                              </Badge>
                            )}
                            {hasExamResult(exam) && exam.is_passed ? (
                              <Badge
                                className={`gap-1 ${statusBadgeClassForStatusValue("SUCCESS")}`}
                              >
                              ...
                              >
                                {(t.examsPage as any).statusNoResult ||
                                  t.examsPage.statusPending}
                              </Badge>
                            ) : (
                              <Badge
                                variant="outline"
                          ...
                          <div className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                            {hasText(exam.confirmation_number) && (
                              <div>
                                <span className="font-medium text-foreground">
                                  {t.examsPage.confirmationNumber}:
                                </span>{" "}
                                {exam.confirmation_number}
                              </div>
                            )}
                            ...
                            {hasText(exam.appointment_start_time) && (
                              <div>
                                <span className="font-medium text-foreground">
                                  {t.examsPage.appointmentStart}:
                                </span>{" "}
                                {formatBackendDate(exam.appointment_start_time)}
                              </div>
                            )}
                            {hasText(exam.appointment_end_time) && (
                              <div>
                                <span className="font-medium text-foreground">
                                  {t.examsPage.appointmentEnd}:
                                </span>{" "}
                                {formatBackendDate(exam.appointment_end_time)}
                              </div>
                            )}
                            {!hasAppointmentDetails(exam) &&
                              !hasExamResult(exam) && (
                                  ...
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                      <div className="font-medium text-blue-800">
                                        {t.examsPage.notScheduledTitle}
                                      </div>
                                      <div className="mt-1 text-xs">
                                        {t.examsPage.notScheduledDesc}
                                      </div>
                                    </div>
                                  </div>
                                </div>
                            ...
                            <div>
                              <span className="font-medium text-foreground">
                                {t.examsPage.candidate}:
                              </span>{" "}
                              {[
                                exam.candidate_first_name,
                                exam.candidate_last_name,
                              ]
                                .filter(Boolean)
                                .join(" ") ||
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {hasExamResult(exam) && exam.exam_id && (
                            <Button variant="outline" asChild>
                              <Link
                                href={`/exams/result?examId=${encodeURIComponent(exam.exam_id)}`}
                              >
