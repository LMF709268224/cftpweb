...
function CourseDetailContent() {
  ...
  const purchased = Boolean(
    detail?.instance && Object.keys(detail.instance).length > 0,
  );
  const instancePipelineId =
    typeof detail?.instance?.pipeline_ulid === "string"
      ? detail.instance.pipeline_ulid
      : "";
    ...
    const loadDetail = async () => {
      ...
      try {
        const res = await apiClient(
          `/api/mall/pipelines/${pipelineId}/runtime`,
        );
        setDetail(res);
      } finally {
        setLoading(false);
  ...
  const firstCourseId = useMemo(
    () =>
      stages
        ...
        .find((unit) => unit.glms_course_id)?.glms_course_id || "",
    [stages],
  );
  const paymentConfigured = Boolean(
    pipeline?.unlock_stripe_price_id || pipeline?.package_stripe_price_id,
  );
  ...
  const activeStageIndex = useMemo(() => {
    ...
    if (nextCourseId) {
      const byCourse = stages.findIndex((stage) =>
        (stage.units || []).some(
          (unit) => unit.glms_course_id === nextCourseId,
        ),
      );
    ...
    const byName = currentStageName
      ? stages.findIndex(
          (stage) => stage.name && stage.name === currentStageName,
        )
      : -1;
  ...
  const stageStateClass = (index: number) => {
    ...
    return timelineStatusBadgeClassForStatus(
      "STAGE",
      stages[index]?.runtime_status,
    );
  };
  const unitStateText = (
    stageIndex: number,
  ...
  const openCertificate = async () => {
    ...
    try {
      const res = await apiClient(
        `/api/pipeline/${instancePipelineId}/certificate-url`,
      );
      if (res?.view_url) {
  ...
  const handleScheduleExam = async () => {
    ...
    try {
      const res = await apiClient(
        `/api/exams/${encodeURIComponent((nextStep as any).exam_id)}/schedule-url?pipeline_ulid=${encodeURIComponent(targetPipelineUlid)}&course_ulid=${encodeURIComponent(nextStep.course_unit_ulid || "")}&url_type=1`,
      );
      if (res?.url) {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      setCourseSummaries(
        Object.fromEntries(
          items.filter(([, course]) => Boolean(course)),
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/mall/courses/${encodeURIComponent(firstCourseId)}/thumbnail-url`,
      {
        credentials: "include",
        headers,
      },
  ...
  const nextStepHref = () => {
    switch (nextStepAction) {
      case "continue_learning":
        return nextStep?.course_id
          ? learningHref(nextStep.course_id)
          : learningHref(firstCourseId);
      case "signup_exam":
          ...
          {loading ? (
            ...
            <>
              <div
                className={cn(
                  "mb-8 grid gap-8",
                  firstCourseThumbnail && "lg:grid-cols-[380px_1fr]",
                )}
              >
                ...
                <div>
                  <div className="mb-3 flex flex-wrap gap-2">
                    ...
                    {pipeline.category_tips && (
                      <Badge variant="outline">{pipeline.category_tips}</Badge>
                    )}
                    {/* {purchased && (
                  ...
                  <h1 className="mb-2 text-2xl font-bold text-foreground">
                    {pipeline.name || t.common.unknownCourse}
                  </h1>
                  ...
                  <div className="mb-6 flex flex-wrap gap-6 text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>
                        {pipeline.final_quals?.length || 0}{" "}
                        {t.credentialsPage.availableQualifications}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      ...
                      disabled={!paymentConfigured || purchased}
                    >
                      <CreditCard className="h-4 w-4" />
                      {purchased
                        ? t.courses.purchased
                        : t.courses.purchasePipeline}
                    </Button>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-3">
                        {!isPipelineTerminal && currentStageName && (
                          <Badge variant="outline">
                            {t.learning.currentStageNameLabel}:{" "}
                            {currentStageName}
                          </Badge>
                        )}
                        {!isPipelineTerminal &&
                        ...
                        {!isPipelineTerminal &&
                          ...
                          currentUnitStatus !== "" && (
                            ...
                            >
                              ...
                              {currentUnitStatusLabel(t, currentUnitStatus)}
                            </Badge>
                          )}
                        {/* <Badge variant="outline">
                        ...
                        {nextStep?.stage_name && (
                          <Badge variant="outline">{nextStep.stage_name}</Badge>
                        )}
                        {nextStep?.course_id &&
                    ...
                    <div className="mt-4 rounded-2xl border bg-card p-4">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Button asChild size="sm" variant="outline">
                          <Link
                            href={`/courses/timeline?id=${encodeURIComponent(pipelineId)}`}
                          >
                            {viewTimelineLabel}
                          </Link>
                        </Button>
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-semibold text-foreground">
                      {t.courses.stageListTitle}
                    </h2>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {t.courses.stageListDesc}
                    </p>
                  </div>
                  <Badge variant="outline">
                    {stages.length} {t.courses.stages} / {totalUnits}{" "}
                    {t.courses.units}
                  </Badge>
                </div>
                {stages.length === 0 ? (
                    ...
                    >
                      <div className="flex flex-col gap-4 border-b px-5 py-4 md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center gap-3">
                          <div
                            className={`flex h-10 w-10 items-center justify-center rounded-xl text-sm font-semibold ${index === activeStageIndex ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary"}`}
                          >
                            {index + 1}
                          </div>
                          <div>
                            <h3 className="font-semibold">
                              {stage.name || `${t.courses.stage} ${index + 1}`}
                            </h3>
                        ...
                        <div className="flex flex-wrap gap-2">
                          <Badge
                            variant="outline"
                            className={stageStateClass(index)}
                          >
                            {t.learning.currentStageStatusLabel}:{" "}
                            {stageStateText(index)}
                          </Badge>
                          <Badge variant="outline">
                            {stageBadgeLabel} {stage.sort_order || index + 1}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <div className="divide-y">
                        {(stage.units || []).map((unit, unitIndex) => {
                          const course = unit.glms_course_id
                            ? courseSummaries[unit.glms_course_id]
                            : null;
                          const unitKey =
                            unit.unit_id ||
                            unit.glms_course_id ||
                          ...
                          const isCurrentUnit =
                            ...
                            (!nextStep?.course_id ||
                              unit.glms_course_id === nextStep.course_id);
                          const canEnterLearning =
                            purchased &&
                            Boolean(unit.glms_course_id) &&
                            (index <= activeStageIndex ||
                          ...
                          const row = (
                            <>
                              <div className="flex items-center gap-3">
                                {canEnterLearning ? (
                                  <div
                                    className={`flex h-8 w-8 items-center justify-center rounded-full ${isCurrentUnit ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary"}`}
                                  >
                              ...
                              <div className="flex flex-wrap items-center justify-end gap-2">
                                ...
                                >
                                  {t.learning.unitStatusLabel}:{" "}
                                  {unitStateText(index, unitIndex, unit)}
                                </Badge>
                                {course?.category_tips && (
                                  <Badge variant="outline">
                                    {course.category_tips}
                          ...
                          return canEnterLearning ? (
                            ...
                            <div
                              key={unitKey}
                              className="flex items-center justify-between gap-4 px-5 py-3 opacity-75"
                            >
                              {row}
                            </div>
                          );
