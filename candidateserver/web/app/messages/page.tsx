...
export default function MessagesPage() {
  ...
  const handleViewDetail = async (message: Message) => {
    try {
      ...
      const res = await apiClient(`/api/messages/${message.id}`)
      setSelectedMessageDetail(res)
                ...
                {filteredMessages.map((message) => {
                  ...
                  return (
                    <div
                      key={message.id}
                      className={cn(
                        "group flex items-start gap-4 p-6 transition-colors hover:bg-muted/50 cursor-pointer",
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="mt-4 pt-4 border-t border-border">
            <div 
              ...
              dangerouslySetInnerHTML={{ __html: selectedMessageDetail?.content || "" }}
            />
          </div>
        </DialogContent>
      </Dialog>
