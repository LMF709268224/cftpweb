...
<template>
  <AppShell content-class="p-4">
    ...
    <div v-else-if="!examId" class="rounded-[22px] bg-white p-8 text-center text-muted-foreground shadow-[0_10px_24px_rgba(15,74,82,0.05)]">{{ t.examsPage.selectExamFirst }}</div>
    <div v-else-if="!result || result.has_result === false" class="rounded-[22px] bg-white p-8 text-center text-muted-foreground shadow-[0_10px_24px_rgba(15,74,82,0.05)]">{{ t.examsPage.noScoreDetails }}</div>
    <div v-else class="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <section class="rounded-[22px] bg-white p-6 shadow-[0_10px_24px_rgba(15,74,82,0.05)]">
      ...
      <section class="rounded-[22px] bg-white p-6 shadow-[0_10px_24px_rgba(15,74,82,0.05)]">
        ...
        <pre v-if="result.score_details_raw" class="max-h-[560px] overflow-auto whitespace-pre-wrap rounded-xl bg-muted/30 p-4 text-xs leading-6 text-muted-foreground">{{ result.score_details_raw }}</pre>
        <div v-else class="rounded-xl bg-muted/30 p-4 text-sm text-muted-foreground">{{ t.examsPage.noScoreDetails }}</div>
      </section>
    </div>
  </AppShell>
</template>
