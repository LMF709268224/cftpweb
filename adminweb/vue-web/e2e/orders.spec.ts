...
test("order detail displays business metadata without a mutation", async ({ page }) => {
  ...
  const originalPriceCard = detailDialog.getByText("计费商品原价", { exact: true }).locator("..")
  const promotionCard = detailDialog.getByText("优惠减免", { exact: true }).locator("..")
  const amountPaidCard = detailDialog.getByText("实付金额", { exact: true }).locator("..")
  await expect(originalPriceCard.getByText("USD 150.00", { exact: true })).toBeVisible()
  await expect(promotionCard.getByText("-USD 25.00", { exact: true })).toBeVisible()
  await expect(amountPaidCard.getByText("USD 129.00", { exact: true })).toBeVisible()
