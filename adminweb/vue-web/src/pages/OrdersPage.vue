...
const zhCopy = {
  ...
  apiNote: "已确认接口：list orders、bundle order detail、bundle order purge。金额只展示列表接口返回的 amount_minor，不做前端兜底推算。",
  refresh: "刷新",
  ...
  search: "查询",
  listTitle: "订单列表",
  listDescription: "来自 `/api/mall/orders`，点击查看详情后在弹框中处理。",
  totalPrefix: "共",
  totalSuffix: "条",
  columns: {
    order: "订单",
  ...
  actionsTitle: "支持操作",
  actionsDescription: "当前 adminbff 只提供认证套餐订单的测试数据清理接口。其他订单类型只读展示。",
  purgeAction: "清理认证测试数据",
  rawNote: "完整字段只读展示，包含列表接口原始字段。",
  confirmTitle: "确认清理认证测试数据",
  confirmDescription: "该操作会调用 `/api/mall/bundle-orders/purge`，用于清理认证套餐订单关联的测试数据。",
  cancel: "取消",
  confirmPurge: "确认清理",
  purging: "清理中...",
  toasts: {
...
const enCopy: typeof zhCopy = {
  ...
  apiNote: "Confirmed APIs: list orders, bundle order detail, bundle order purge. Amounts only use amount_minor returned by the list API; no frontend fallback calculation.",
  refresh: "Refresh",
  ...
  listDescription: "From `/api/mall/orders`; click details to handle it in a dialog.",
  totalPrefix: "Total",
  totalSuffix: "items",
  columns: {
    order: "Order",
  ...
  actionsDescription: "The current adminbff only provides a test data purge API for certification bundle orders. Other order types are read-only.",
  purgeAction: "Purge Certification Test Data",
  rawNote: "Raw fields are read-only and include the original fields returned by the list API.",
  confirmTitle: "Confirm Test Data Purge",
  confirmDescription: "This will call `/api/mall/bundle-orders/purge` to clean test data associated with the certification bundle order.",
  cancel: "Cancel",
