  ...
  <section class="mx-auto flex min-h-screen w-full max-w-[1480px] flex-col gap-6 px-8 py-8">
    ...
    <div class="grid gap-6 xl:grid-cols-[460px_minmax(0,1fr)]">
      <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-200 p-5">
          <div>
            ...
            <p class="mt-1 text-sm text-slate-500">来自 `/api/mall/invoices`。</p>
          </div>
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              <div class="truncate font-black">{{ invoiceId(invoice) || "-" }}</div>
              <div class="mt-1 break-all text-sm text-slate-500">订单：{{ orderId(invoice) }}</div>
            </div>
            <span class="shrink-0 rounded-full border px-3 py-1 text-xs font-black" :class="badgeClass(invoice.status)">{{ invoice.status || "-" }}</span>
          </div>
          <div class="mt-3 text-sm font-bold">{{ amountText(invoice) }}</div>
          <div class="mt-1 text-xs text-slate-400">{{ formatDate(String(invoice.created_at || "")) }}</div>
        </button>
        <div v-if="!loading && !invoices.length" class="p-12 text-center text-slate-500">暂无发票</div>
      ...
      <section class="rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div v-if="!selected" class="p-12 text-center text-slate-500">请选择一张发票</div>
        <template v-else>
          ...
          <div class="space-y-5 p-5">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <label v-for="(value, key) in selectedFields" :key="key" class="grid gap-2 text-sm font-bold">
                {{ key }}
                <input class="rounded-xl border border-slate-200 bg-slate-100 px-4 py-3 text-slate-600" disabled :value="String(value ?? '-')" />
              </label>
            </div>
