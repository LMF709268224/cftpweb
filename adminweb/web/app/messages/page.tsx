...
export default function AdminMessagesPage() {
  ...
  const getMessageStatusText = (status: any) => {
    return statusLabelWithDiagnostics(t, MESSAGE_STATUS_LABELS, MESSAGE_STATUS_ENUM_NAMES, status, "messagesPage.statusUnread")
  }
  ...
  useEffect(() => {
    if (templateId) {
      const fetchTemplateDetail = async () => {
        try {
          const res = await apiClient(`/api/messages/templates/detail?path=${encodeURIComponent(templateId)}`)
          if (res && res.title_tpl) {
            const matches = [...(res.title_tpl.matchAll(/\{\{([^}]+)\}\}/g)), ...(res.content_tpl?.matchAll(/\{\{([^}]+)\}\}/g) || [])];
  ...
  const handleEditClick = async (tpl: any) => {
    ...
    setNewTplTitle(t.common?.loading || "Loading...")
    setNewTplContent("")
    setNewTplDesc(tpl.description || "")

    try {
      const res = await apiClient(`/api/messages/templates/detail?path=${encodeURIComponent(path)}`)
      if (res) {
        setNewTplTitle(res.title_tpl || "")
        setNewTplContent(res.content_tpl || "")
      }
    } catch (err) {
      console.error("Failed to fetch template detail", err)
  ...
  const handleSend = async () => {
    ...
    // Validate payload is valid JSON object if not empty
    if (payload.trim() !== "") {
      try {
                      ...
                      <div key={msg.message_id || i} className="border-b pb-4 last:border-0">
                        ...
                        >
                          <div className="font-medium text-sm">
                            ...
                            {t.messagesPage.userIdShort}: {msg.user_id} - {t.messagesPage.templateShort}: {msg.template_path || msg.template_id}
                          </div>
                          <div className="flex items-center gap-3">
                            <Badge variant="outline" className={statusBadgeClassForStatus(MESSAGE_STATUS_ENUM_NAMES, msg.status)}>
                        ...
                        {isExpanded && (
                          <div className="mt-3 bg-muted/30 p-3 rounded text-sm font-mono whitespace-pre-wrap break-words">
                            <div className="mb-2 pb-2 border-b border-muted">
                              <span className="font-semibold">{t.messagesPage.messageId}:</span> {msg.message_id}
                            </div>
                            <div className="mb-2 pb-2 border-b border-muted">
                              <span className="font-semibold">{t.messagesPage.sentAt}:</span> {msg.created_at}
                            </div>
                            <div className="flex gap-4">
                              <div><span className="font-semibold">{t.messagesPage.messageTypeShort}:</span> {msg.msg_type}</div>
                              <div><span className="font-semibold">{t.messagesPage.messageSource}:</span> {msg.msg_source}</div>
                            </div>
                          </div>
                        )}
                      </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="bg-card rounded-xl border p-6">
                ...
                <div className="space-y-4">
                  {!editingTemplateId && (
                    <div>
                      <label className="block text-sm font-medium mb-1">{t.messagesPage.templatePath}</label>
                      <input
                        type="text"
                        value={newTplPath}
                        onChange={(e) => setNewTplPath(e.target.value)}
                        placeholder={t.messagesPage.templatePathPlaceholder}
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">{t.messagesPage.templateDescription}</label>
                    <input
                      type="text"
                      value={newTplDesc}
                      onChange={(e) => setNewTplDesc(e.target.value)}
                      placeholder={t.messagesPage.templateDescriptionPlaceholder}
                      className="w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm"
                    />
                  </div>
                  <div className="flex gap-2">
                      ...
                      return (
                        <div key={path} className="border-b pb-4 last:border-0 relative">
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-mono text-xs bg-muted px-2 py-1 rounded select-all cursor-pointer" title={t.messagesPage.templatePathCopyTitle} onClick={(e) => {
