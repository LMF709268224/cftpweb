...
export default function ApplicationsPage() {
  ...
  const getStatusDisplay = (status: string) => {
    ...
    if (upperStatus.includes("REJECTED")) {
      return <Badge className={`hover:bg-transparent ${statusBadgeClassForStatusValue("REJECTED")}`} variant="outline">{t.applicationsPage.statusRejected}</Badge>
    }
    if (upperStatus.includes("APPROVED")) {
      return <Badge className={`hover:bg-transparent ${statusBadgeClassForStatusValue("APPROVED")}`} variant="outline">{t.applicationsPage.statusApproved}</Badge>
    }
    if (upperStatus.includes("RESUBMIT") || upperStatus.includes("REUPLOAD")) {
      return <Badge className={`hover:bg-transparent ${statusBadgeClassForStatusValue("RESUBMIT")}`} variant="outline">{t.applicationsPage.statusResubmit}</Badge>
    }
    if (upperStatus.includes("PENDING") || upperStatus === "1") {
      return <Badge className={`hover:bg-transparent ${statusBadgeClassForStatusValue("PENDING")}`} variant="outline">{t.applicationsPage.statusPending}</Badge>
    }
